...
`endif
/* 
    logic [1023:0] j_mem_bank [0:`PARALLELISM-1][0:255];
    // Register to delay weight_ready_o by 1 cycle
    logic weight_ready_o_d;

    // Symmetric J matrix: 256x256, 4-bit elements
    logic [3:0] J_matrix [0:255][0:255];

    // Initialize J_matrix with symmetry (J[i][j] = J[j][i])
    initial begin
        int bank, addr;
        for (int i = 0; i < 256; i++) begin
            for (int j = 0; j < 256; j++) begin
                if (j < i)
                    J_matrix[i][j] = J_matrix[j][i]; // enforce symmetry
                else if (j == i)
                    J_matrix[i][j] = 4'd0; // diagonal can be zero or any value
                else
                    J_matrix[i][j] = $urandom_range(0, 15); // random 4-bit value
            end
        end

        // Populate pseudo memory banks with J matrix columns
        for (int col = 0; col < 256; col++) begin
            ...
            for (int row = 0; row < 256; row++) begin
                j_mem_bank[bank][addr][row*4 +: 4] = J_matrix[row][col];
            end
        end
    end
 // Testbench parameters
    ...
    // ========================================================================
    logic [31:0] debug_logic_ctrl_current_state;
    logic [31:0] debug_logic_ctrl_next_state;
    logic debug_logic_ctrl_spin_handshake;
    logic debug_logic_ctrl_weight_handshake;
    logic debug_logic_ctrl_energy_handshake;
    logic debug_logic_ctrl_energy_valid_comb;
    logic debug_logic_ctrl_energy_valid_reg;
    logic [`PIPESMID:0] debug_logic_ctrl_counter_ready_pipe;
    logic [2:0] debug_logic_ctrl_spin_handshake_pipe;
    
    // Hierarchical signal assignments for debug monitoring
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            debug_logic_ctrl_current_state <= '0;
            debug_logic_ctrl_next_state <= '0;
            debug_logic_ctrl_spin_handshake <= '0;
            debug_logic_ctrl_weight_handshake <= '0;
            debug_logic_ctrl_energy_handshake <= '0;
            debug_logic_ctrl_energy_valid_comb <= '0;
            debug_logic_ctrl_energy_valid_reg <= '0;
            debug_logic_ctrl_counter_ready_pipe <= '0;
            debug_logic_ctrl_spin_handshake_pipe <= '0;
        end else begin
            // Capture internal signals from logic_ctrl using hierarchical references
            debug_logic_ctrl_current_state <= dut.u_logic_ctrl.current_state;
            debug_logic_ctrl_next_state <= dut.u_logic_ctrl.next_state;
            debug_logic_ctrl_spin_handshake <= dut.u_logic_ctrl.spin_handshake;
            debug_logic_ctrl_weight_handshake <= dut.u_logic_ctrl.weight_handshake;
            debug_logic_ctrl_energy_handshake <= dut.u_logic_ctrl.energy_handshake;
            debug_logic_ctrl_energy_valid_comb <= dut.u_logic_ctrl.energy_valid_comb;
            debug_logic_ctrl_energy_valid_reg <= dut.u_logic_ctrl.energy_valid_reg;
            debug_logic_ctrl_counter_ready_pipe <= dut.u_logic_ctrl.counter_ready_pipe;
            debug_logic_ctrl_spin_handshake_pipe <= dut.u_logic_ctrl.spin_handshake_pipe;
        end
    end
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni) begin
            ...
            // Reference calculations and counters remain unchanged
            ...
            // Minimal change: energy_ready_i is asserted one cycle after energy_valid_o
            energy_ready_i <= energy_valid_o;
        end
    end
    ...
    // Task to handle weight input with 1-cycle delayed valid after ready
    task automatic weight_interface();
        begin
            ...
            forever begin
                ...
                // Use DUT's weight_raddr_em_o output for memory bank addressing
                for (int i = 0; i < `PARALLELISM; i++) begin
                    int addr = dut.weight_raddr_em_o[i];
                    int bank = i;
                    for (int row = 0; row < DATASPIN; row++) begin
                        if (LITTLE_ENDIAN == 1) begin
                            // Little-endian: spin 0 at lowest bits
                            weight_i[i*DATASPIN*BITJ + row*BITJ +: BITJ] = j_mem_bank[bank][addr][row*BITJ +: BITJ];
                        end else begin
                            // Big-endian: spin 255 at lowest bits
                            weight_i[i*DATASPIN*BITJ + (DATASPIN-1-row)*BITJ +: BITJ] = j_mem_bank[bank][addr][row*BITJ +: BITJ];
                        end
                    end
                end

                // Prepare hbias and hscaling as before (can be updated for banked storage later)
                for (int i = 0; i < `PARALLELISM; i++) begin
                    case(`test_mode)
                ...
                // Wait for weight_ready_o to go high, then assert valid in the next cycle
                wait(weight_ready_o);
                @(posedge clk_i);
                weight_valid_i = 1;
    ...
    endtask
    // Delay weight_ready_o by 1 cycle (for use elsewhere if needed)
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (!rst_ni)
            weight_ready_o_d <= 0;
        else
            weight_ready_o_d <= weight_ready_o;
    end
