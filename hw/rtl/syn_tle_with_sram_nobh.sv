// Thin variant of syn_tle_with_sram used only by the TB to force hbias/hscaling = 0
// This is a local copy that disables hbias/hscaling SRAM reads.
`ifndef True
`define True 1'b1
`endif
...
module syn_tle_with_sram_nobh #(
    ...
    // Keep most logic identical to main wrapper but zero hbias/hscaling
    ...
    generate
        ...
        // Instead of instantiating hbias/scaling SRAMs, set them to zero for TB
        for (i = 0; i < PARALLELISM; i++) begin : gen_hbias_scaling_zero
            assign sram_hbias[i*BITH +: BITH] = '0;
            assign sram_hscaling[i*SCALING_BIT +: SCALING_BIT] = '0;
        end
    endgenerate
